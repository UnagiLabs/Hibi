...
このJSONには、アルバムのタイトル、対象月、MemWalから見つけたハイライト、育児ログの要約、Walrusに保存した写真の参照が入る。
...
| Walrus | 写真BlobとアルバムManifestなど、長く残したい成果物の保存 |
...
4. 対象月のMediaAssetをDBから取得する。
5. `AlbumManifest` JSONを生成する。
6. JSONのsha256 hashを計算する。
7. `SUI_PRIVATE_KEY` があればWalrus testnetへ保存する。
8. DBの `Album.manifestSha256` と `Album.manifestWalrusBlobId` を更新する。
...
写真は `POST /api/photos` で受け取り、WalrusへBlobとして保存する。

```sh
curl -X POST http://127.0.0.1:4000/api/photos \
  -F 'caption=はじめて寝返りした' \
  -F 'occurredAt=2026-06-05T10:00:00.000Z' \
  -F 'file=@/path/to/photo.png;type=image/png'
```

レスポンスの見方:

- `mediaAsset.walrusBlobId`: 写真BlobのBlob ID。
- `mediaAsset.url`: Hibi API経由で写真を読むURL。
- `walrus.status: "done"`: 写真BlobのWalrus保存成功。
- `memoryItemId`: captionを思い出としてDBに保存したID。
- `memwal.status: "done"`: captionをMemWalに保存成功。

月次アルバムを再生成すると、対象月の写真が `AlbumManifest` に含まれ、Webの月次アルバムにも表示される。
